...
galaxy_data = list(np.loadtxt('simulation_field.txt', delimiter = ' '))
...
ng_max = 500 #maximu number of galaxies in a cluster
...
for i in range(0, nclust-1):
    add_cluster((cluster_locs[0][i], cluster_locs[1][i]),
                ang_cluster/2, ngal[i])
    
found_clusters = get_clusters(galaxy_data, p, z, h, minz, linking,
                              'found_clusters.txt')

#now compare the found clusters with the natural clusters to determine
#completeness and false positives
#these are the clusters found by the algorithms
# found_clusters = list(np.loadtxt('found_clusters.txt', delimiter = " ",
#                                  skiprows = 1, usecols = (0, 1)))
# natural_clusters = list(np.loadtxt('natural_clusters.txt',
#                                    delimiter = " ", skiprows = 1,
#                                    usecols = (0, 1)))


n_catalog = SkyCoord(natural_clusters[0]*u.degree,
                     natural_clusters[1]*u.degree)
l_catalog = SkyCoord(cluster_locs[0]*u.degree,
                     cluster_locs[1]*u.degree)
f_catalog = SkyCoord(found_clusters[0]*u.degree,
                     found_clusters[1]*u.degree)

n_idx = np.empty(shape = (len(found_clusters[0]), 1))
n_sep2d = np.empty(shape = (len(found_clusters[0]), 1))
l_idx = np.empty(shape = (len(found_clusters[0]), 1))
l_sep2d = np.empty(shape = (len(found_clusters[0]), 1))


for j in range(0, len(found_clusters[0]) - 1):
